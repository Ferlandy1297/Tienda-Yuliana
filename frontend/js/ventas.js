...
import { ventasApi, pagosVentaApi, productosApi } from './api.js';
...
function setRowProductInfo(row, prod){
  const idInput = row.querySelector('.idProducto');
  if (idInput && prod?.id) idInput.value = Number(prod.id);
...
function renderResults(list){
  ...
  list.slice(0, 8).forEach(p => {
    ...
    li.className = 'hstack';
    li.innerHTML = `<span class="tag">${p.id}</span> <span>${p.nombre}</span> <span class="tag right">${p.codigoBarras}</span>`;
...
async function loadHist(){
  ...
  list.forEach(v => {
    ...
    tr.innerHTML = `<td>${v.id}</td><td>${v.fechaHora}</td><td>${v.tipo}</td><td>${v.estado}</td><td>${v.total}</td>`;
...
document.getElementById('btnPagarVenta')?.addEventListener('click', async ()=>{
  ...
  if (!idVenta) { pvFeedback.textContent='ID Venta requerido'; pvFeedback.className='error'; return; }
